        ...
        <div class="card-body p-3">
            <form method="GET" action="{{ route('activos.index') }}" class="row g-3 align-items-center">
                ...
                <div class="col-md-1 text-end">
                    <a href="{{ route('activos.index') }}" class="btn btn-light text-muted" title="Limpiar"><i class="bi bi-arrow-counterclockwise"></i></a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card card-dashboard border-0 shadow-sm">
        <div class="card-body p-0">
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="border-bottom">
                            ...
                            <td class="py-3">
                                <div class="badge bg-light text-dark border fw-normal mb-1">{{ $activo->tipo->nombre ?? 'N/A' }}</div>
                                <div class="small text-dark fw-bold">{{ $activo->marca->nombre ?? '' }} {{ $activo->modelo }}</div>
                            </td>

                            <td class="py-3 small">
                                @if(isset($activo->especificaciones['procesador'])) 
                                    <div class="d-flex align-items-center text-muted mb-1">
                                        <i class="bi bi-cpu me-2"></i> {{ $activo->especificaciones['procesador'] }}
                                    </div>
                                    @if(isset($activo->especificaciones['ram']))
                                        <div class="d-flex align-items-center text-muted">
                                            <i class="bi bi-memory me-2"></i> {{ $activo->especificaciones['ram'] }}
                                        </div>
                                    @endif
                                @elseif(isset($activo->especificaciones['imei']))
                                    <div class="d-flex align-items-center text-muted">
                                        <i class="bi bi-phone me-2"></i> IMEI: <span class="font-monospace ms-1">{{ $activo->especificaciones['imei'] }}</span>
                                    </div>
                                    @if(isset($activo->especificaciones['pantalla']))
                                        <div class="text-muted ms-4"><small>{{ $activo->especificaciones['pantalla'] }}"</small></div>
                                    @endif
                                @elseif(isset($activo->especificaciones['otras']) && !empty($activo->especificaciones['otras']))
                                    <div class="text-muted text-truncate" style="max-width: 200px;">
                                        <i class="bi bi-card-text me-1"></i> {{ $activo->especificaciones['otras'] }}
                                    </div>
                                @else
                            ...
                            <td class="pe-4 py-3 text-end">
                                <button class="btn btn-sm btn-light text-primary rounded-circle me-1" title="Ver Detalles" onclick="verActivo('{{ $activo->id }}')">
                                    <i class="bi bi-eye"></i>
                                </button>
                                <button class="btn btn-sm btn-light text-info rounded-circle me-1" title="Editar" onclick="editarActivo('{{ $activo->id }}')">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                <button class="btn btn-sm btn-light text-danger rounded-circle" title="Dar de Baja Definitiva" onclick="prepararBaja('{{ $activo->id }}', '{{ $activo->codigo_interno }}')">
            ...
            <div class="d-flex justify-content-between align-items-center p-3 border-top">
                <small class="text-muted">Mostrando {{ $activos->firstItem() }}-{{ $activos->lastItem() }} de {{ $activos->total() }}</small>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-danger text-white">
                <h6 class="modal-title fw-bold"><i class="bi bi-exclamation-triangle-fill me-2"></i>Baja Definitiva de Activo</h6>
