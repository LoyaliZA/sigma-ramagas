        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-3 gap-3">
            <ul class="nav nav-pills" id="pills-tab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active rounded-pill px-4" id="pills-activas-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-activas" type="button" role="tab">
                        <i class="bi bi-person-workspace me-2"></i>Asignaciones Activas
                        ({{ $asignacionesActivas->count() }})
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link rounded-pill px-4" id="pills-historial-tab" data-bs-toggle="pill"
            ...
            <form action="{{ route('asignaciones.index') }}" method="GET" class="d-flex position-relative"
                style="max-width: 300px; width: 100%;">
                <input type="text" name="search" class="form-control ps-5 rounded-pill bg-white border-0 shadow-sm"
                    placeholder="Buscar activo, empleado..." value="{{ request('search') }}">
                <i class="bi bi-search position-absolute top-50 start-0 translate-middle-y ms-3 text-muted"></i>
                @if(request('search'))
                    <a href="{{ route('asignaciones.index') }}"
                        class="position-absolute top-50 end-0 translate-middle-y me-3 text-muted"><i
                                        ...
                                        <tr>
                                            <td class="ps-4">
                                                <div class="d-flex flex-column">
                                                    ...
                                                    <small class="text-dark">{{ $asig->activo->modelo }}
                                                        ({{ $asig->activo->numero_serie }})</small>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex flex-column">
                                                    <span class="fw-semibold text-dark">{{ $asig->empleado->nombre }}
                                                        {{ $asig->empleado->apellido_paterno }}</span>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <button
                                                        class="btn btn-sm btn-soft-warning rounded-pill px-3 btn-subir-documento"
                                                        data-id="{{ $asig->id }}">
                                            ...
                                            <td class="text-end pe-4">
                                                <div class="btn-group">
                                                    ...
                                                    @if($asig->lote_id)
                                                        <a href="{{ route('asignaciones.carta_lote', $asig->lote_id) }}"
                                                            target="_blank" class="btn btn-outline-secondary btn-sm"
                                                            title="Ver Formato PDF (Lote)">
                                                            <i class="bi bi-file-pdf"></i>
                                                        </a>
                                                    @else
                                                        <a href="{{ route('asignaciones.carta', $asig->id) }}" target="_blank"
                                                            class="btn btn-outline-secondary btn-sm"
                                                            title="Ver Formato PDF (Individual)">
                                                            <i class="bi bi-file-pdf"></i>
                                                        </a>
                                                    @endif

                                                    <button type="button"
                                                        class="btn btn-outline-danger btn-sm btn-devolver-trigger"
                                                        data-id="{{ $asig->id }}" data-serie="{{ $asig->activo->numero_serie }}"
                            ...
                            <table class="table table-hover align-middle mb-0">
                                <thead class="bg-light text-uppercase small text-muted">
                                    <tr>
                                        ...
                                        <th class="py-3">Fechas (Asig. / Dev.)</th>
                                ...
                                <tbody>
                                    @forelse($historial as $hist)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="fw-semibold">{{ $hist->empleado->nombre }}
                                                    {{ $hist->empleado->apellido_paterno }}</span>
                                            </td>
                                            <td>
                                                <small class="d-block text-muted">Del:
                                                    {{ $hist->fecha_asignacion->format('d/m/Y') }}</small>
                                                <small class="d-block text-dark fw-bold">Al:
                                                    {{ $hist->fecha_devolucion->format('d/m/Y') }}</small>
                                            </td>
                                            <td>
                                            ...
                                            <td style="max-width: 200px;">
                                                <span class="text-muted small text-truncate d-block"
                                                    title="{{ $hist->observaciones_devolucion }}">
                                                    {{ $hist->observaciones_devolucion ?? 'Sin observaciones' }}
                                                </span>
                                            </td>
                                            <td class="text-end pe-4">
                                                @if($hist->fecha_devolucion)
                                                    <a href="{{ route('asignaciones.carta_devolucion', $hist->id) }}"
                                                        target="_blank" class="btn btn-sm btn-light text-danger"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-5 text-muted">No hay historial disponible.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                ...
                <form id="formAsignar">
                    <div class="modal-body">
                        <div class="mb-4 p-3 bg-light rounded-3 border">
                            <label class="form-label fw-bold small text-uppercase text-muted">1. Seleccionar Empleado
                                *</label>
                            <select class="form-select form-select-lg" name="empleado_id" required>
                                ...
                                @foreach($empleados as $emp)
                                    <option value="{{ $emp->id }}">{{ $emp->numero_empleado }} - {{ $emp->nombre }}
                        ...
                        <label class="form-label fw-bold small text-uppercase text-muted">2. Seleccionar Activos a Entregar
                            *</label>
                        <div id="lista-activos-container">
                            <div class="row g-2 mb-2 activo-row align-items-center">
                                <div class="col-10">
                                    <select class="form-select select-activo" name="activos[]" required
                                        onchange="verificarDuplicados(this)">
                                        <option value="" selected disabled>Seleccionar Activo...</option>
                                        @foreach($activosDisponibles as $act)
                                            <option value="{{ $act->id }}">{{ $act->tipo->nombre }} - {{ $act->modelo }}
                                                ({{ $act->numero_serie }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-2">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" name="fecha_asignacion" value="{{ date('Y-m-d') }}"
                                    required>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-12 mt-3">
                                ...
                                <textarea class="form-control" name="observaciones" rows="2"
                                    placeholder="Cargadores, condiciones, etc..."></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer border-0 pt-0">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save me-2"></i>Guardar y Generar
                            PDF</button>
                    </div>
                </form>
            </div>
                ...
                <form id="formDevolver">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-light" value="{{ date('d/m/Y') }} (Automático)"
                                disabled readonly>
                            <small class="text-muted" style="font-size: 0.75rem;">
                                * La fecha se registra automáticamente al momento de confirmar.
                            </small>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="observaciones" rows="3" required
                                placeholder="Describe el estado del activo..."></textarea>
                        </div>
                    </div>
            ...
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body">
                    <ul class="nav nav-tabs mb-3" id="docTabs" role="tablist">
                        <li class="nav-item">
                            <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab-subir">Subir
                                Firmado</button>
                        </li>
                        <li class="nav-item">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-historial"
                                id="btnLoadHistorial">Historial</button>
                        </li>
                    </ul>
                        ...
                        <div class="tab-pane fade show active" id="tab-subir">
                            <form id="formSubirDoc">
                                ...
                                <div class="mb-3 text-center p-4 border rounded-3 bg-light border-dashed">
                                    ...
                                    <div class="mt-2">
                                        <label for="archivoInput" class="btn btn-sm btn-primary cursor-pointer">Seleccionar
                                            PDF/Imagen</label>
                                        <input type="file" name="documento" id="archivoInput" class="d-none"
                                            accept=".pdf,.jpg,.png" required onchange="mostrarNombreArchivo(this)">
                                    </div>
                                    <small class="d-block mt-2 text-muted" id="nombreArchivoSel">Ningún archivo
                                        seleccionado</small>
                                </div>
            ...
            window.mostrarNombreArchivo = function (input) {
                ...
                if (archivo) {
                    ...
                    if (archivo.size > limite) {
                        alert('El archivo es demasiado pesado. El tamaño máximo permitido es 1MB.');
                        input.value = ''; // Borramos el archivo seleccionado
                        document.getElementById('nombreArchivoSel').textContent = 'Ningún archivo seleccionado';
                        return; // Salimos de la función
                    }

                    // Si pasa la validación, mostramos el nombre
                    document.getElementById('nombreArchivoSel').textContent = archivo.name;
                } else {
                    document.getElementById('nombreArchivoSel').textContent = 'Ningún archivo seleccionado';
                }
            };
