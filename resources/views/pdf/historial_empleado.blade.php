...
<head>
    ...
    <style>
        ...
        .section-title { font-weight: bold; background-color: #f0f0f0; padding: 5px; border-left: 4px solid #ce1126; margin-bottom: 10px; font-size: 12px; }
...
<body>
    <div class="header">
        ...
        <h2 style="margin: 5px 0;">HISTORIAL DE ACTIVOS</h2>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $asig->activo->numero_serie }}</td>
                <td>{{ $asig->activo->tipo->nombre }}<br><small>{{ $asig->activo->marca->nombre }}</small></td>
                <td>{{ $asig->activo->modelo }}</td>
                <td>{{ \Carbon\Carbon::parse($asig->fecha_asignacion)->format('d/m/Y') }}</td>
                <td>{{ $asig->fecha_devolucion ? \Carbon\Carbon::parse($asig->fecha_devolucion)->format('d/m/Y') : '-' }}</td>
                <td>
                    @if($asig->fecha_devolucion)
                        <span style="color:#856404; font-style:italic;">Devuelto</span>
                    @else
                        <span style="color:#155724; font-weight:bold;">ACTIVO</span>
                    @endif
                </td>
            </tr>
            @empty
            <tr><td colspan="6" style="text-align:center;">Sin registros históricos.</td></tr>
            @endforelse
        </tbody>
    </table>

    <div class="footer">SIGMA - Sistema de Gestión de Activos | Grupo Ramagas S.A de C.V.</div>
</body>
</html>